...
> 📍 **入口说明**：AstrBot 支持原生 Plugin Pages 时，管理端会优先接入 AstrBot WebUI：在 `插件` 页面进入本插件详情页即可打开，无需独立端口，鉴权由 Dashboard 承担；旧版 AstrBot 会自动回退为独立端口模式（默认 `http://127.0.0.1:4100`，可在插件配置中修改）。
...
这部分控制的是插件自带 WebUI 是否启动、监听在哪个地址、是否启用访问密码。AstrBot 支持原生 Plugin Pages 时，管理端会优先从 WebUI 插件详情页进入（无需独立端口）；下方地址/端口/密码仅在回退的独立端口模式下生效。

- **启用 Web 管理端 (`enabled`)**:
  - 类型：`Boolean`
  - 默认值：`true`
  - 说明：开启后，插件会优先接入 AstrBot 原生 Plugin Pages；不支持时回退启动独立的 Web 管理端，用于查看状态、管理任务、浏览文档、编辑配置等。
